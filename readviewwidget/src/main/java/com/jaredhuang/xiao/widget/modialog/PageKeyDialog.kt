...
class PageKeyDialog(context: Context) : BaseDialog(context) {
    ...
    init {
        ...
        et_prev.setText(ReadDataExt.getInstance().getConfigPreferences().getInt("prevKeyCode", 0).toString())
        et_next.setText(ReadDataExt.getInstance().getConfigPreferences().getInt("nextKeyCode", 0).toString())
        tv_ok.onClick {
            val edit = ReadDataExt.getInstance().getConfigPreferences().edit()
            et_prev.text?.let {

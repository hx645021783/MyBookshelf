...
import com.jaredhuang.xiao.help.BlurTransformation;
...
import com.jaredhuang.xiao.widget.CoverImageView;
...
import static com.jaredhuang.xiao.presenter.BookDetailPresenter.FROM_BOOKSHELF;
...
public class BookDetailActivity extends MBaseActivity<BookDetailContract.Presenter> implements BookDetailContract.View {
    ...
    public void updateView() {
        ...
        if (null != bookCollectBean) {
            ...
            bookInfoBean = bookCollectBean.getBookInfoBean();
            tvName.setText(bookInfoBean.getName());
            author = bookInfoBean.getAuthor();
            tvAuthor.setText(TextUtils.isEmpty(author) ? "未知" : author);
            ((RadioButton) rgBookGroup.getChildAt(bookCollectBean.getGroup())).setChecked(true);
            if (mPresenter.getInBookShelf()) {
                tvChapter.setText(bookCollectBean.getDurChapterName()); // last
                ...
                if (!TextUtils.isEmpty(bookCollectBean.getLastChapterName())) {
                    tvChapter.setText(bookCollectBean.getLastChapterName()); // last
                }
                tvShelf.setText(R.string.add_to_shelf);
                tvRead.setText(R.string.start_read);
                tvShelf.setOnClickListener(v -> {
            ...
            if (!TextUtils.isEmpty(bookCollectBean.getCustomCoverPath())) {
                upImageView(bookCollectBean.getCustomCoverPath());
            } else {
    ...
    protected void bindEvent() {
        tvName.setOnClickListener(v -> {
            if (bookCollectBean == null) return;
            if (TextUtils.isEmpty(bookCollectBean.getBookInfoBean().getName())) return;
            if (!AppActivityManager.getInstance().isExist(SearchBookActivity.class)) {
                SearchBookActivity.startByKey(this, bookCollectBean.getBookInfoBean().getName());
            } else {
                RxBus.get().post(RxBusTag.SEARCH_BOOK, bookCollectBean.getBookInfoBean().getName());
            }
            finish();
        });
        ...
        tvRead.setOnClickListener(v -> {
            if (!mPresenter.getInBookShelf()) {
                BookCollectHelp.saveBookToShelf(mPresenter.getBookShelf());
    ...
    private void upChapterSizeTv() {
        String chapterSize = "";
        if (mPresenter.getOpenFrom() == FROM_BOOKSHELF && bookCollectBean.getChapterListSize() > 0) {
            int newChapterNum = bookCollectBean.getChapterListSize() - 1 - bookCollectBean.getDurChapter();
            if (newChapterNum > 0)

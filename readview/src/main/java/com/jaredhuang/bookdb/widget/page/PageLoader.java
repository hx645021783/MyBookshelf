package com.jaredhuang.bookdb.widget.page;
...
public abstract class PageLoader {
    ...
    /*****************************init params*******************************/
    public PageLoader(PageView pageView, BookShelfBean book, Callback callback) {
        mPageView = pageView;
        this.book = book;
     ...
     * 加载错误
     */
    protected void durDhapterError(String msg) {
     ...
     * 打开当前章节指定页
     */
    protected void openChapter(int pagePos) {
        mCurPagePos = pagePos;
    ...
    private void upPage() {
        if (mPageMode != PageAnimation.Mode.SCROLL) {
            ...
            if (mCurPagePos < curChapter().txtChapter.getPageSize() - 1 || curChapter().txtChapter.getPosition() < bookChapterBeanList.size() - 1) {
                mPageView.drawPage(1);
            }
        }
    }
    ...
    synchronized void drawPage(Bitmap bitmap, int pageOnCur) {
        ...
        if (pageOnCur == 0) { //当前页
            ...
            if (mCurPagePos + 1 < curChapter().txtChapter.getPageSize()) {
                ...
                if (mCurChapterPos + 1 >= bookChapterBeanList.size()) {
                    txtChapter = new TxtChapter(mCurChapterPos + 1);
                    txtChapter.setStatus(TxtChapter.Status.ERROR);
    ...
    private synchronized void drawBackground(final Canvas canvas, TxtChapter txtChapter, TxtPage txtPage) {
        ...
        if (!bookChapterBeanList.isEmpty()) {
            String title = bookChapterBeanList.size() > txtChapter.getPosition() ? bookChapterBeanList.get(txtChapter.getPosition()).getDurChapterName() : "";
    ...
    void drawContent(final Canvas canvas, float offset) {
        ...
        while (top < totalHeight) {
            ...
            for (int i = page.getTitleLines(); i < page.size(); i++) {
                ...
                isLight = readAloudParagraph == chapter.getParagraphIndex(paragraphLength);
//                isLight = ReadAloudService.running && readAloudParagraph == chapter.getParagraphIndex(paragraphLength);
    ...
    protected void parseCurChapter() {
        if (curChapter().txtChapter.getStatus() != TxtChapter.Status.FINISH) {
            Single.create((SingleOnSubscribe<TxtChapter>) e -> {
                ...
                TxtChapter txtChapter = chapterProvider.dealLoadPageList(bookChapterBeanList.get(mCurChapterPos), mPageView.isPrepare());
    ...
    protected void parsePrevChapter() {
        ...
        Single.create((SingleOnSubscribe<TxtChapter>) e -> {
            ChapterProvider chapterProvider = new ChapterProvider(this);
            TxtChapter txtChapter = chapterProvider.dealLoadPageList(bookChapterBeanList.get(prevChapterPos), mPageView.isPrepare());
            e.onSuccess(txtChapter);
        })
                .compose(RxUtils::toSimpleSingle)
                .subscribe(new SingleObserver<TxtChapter>() {
    ...
    protected void parseNextChapter() {
        ...
        if (nextChapterPos >= bookChapterBeanList.size()) {
            nextChapter().txtChapter = null;
            return;
        }
        ...
        Single.create((SingleOnSubscribe<TxtChapter>) e -> {
            ChapterProvider chapterProvider = new ChapterProvider(this);
            TxtChapter txtChapter = chapterProvider.dealLoadPageList(bookChapterBeanList.get(nextChapterPos), mPageView.isPrepare());
            e.onSuccess(txtChapter);
        })
                .compose(RxUtils::toSimpleSingle)
                .subscribe(new SingleObserver<TxtChapter>() {
    ...
    public interface Callback {
        ...
        Observable<List<BookChapterBean>> getChapterList(BookShelfBean book);
        ObservableSource<BookContentBean> getBookContent(BookShelfBean book, BookChapterBean bookChapterBean, BaseChapterBean nextChapterBean);

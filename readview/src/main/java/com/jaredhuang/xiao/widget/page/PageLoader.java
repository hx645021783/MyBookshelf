package com.jaredhuang.xiao.widget.page;
...
public abstract class PageLoader {
    ...
    public void setOnNetLoaderCallback(OnNetLoaderCallback onNetLoaderCallback) {
        this.onNetLoaderCallback = onNetLoaderCallback;
    }
...
//                isLight = ReadAloudService.running && readAloudParagraph == chapter.getParagraphIndex(paragraphLength);
    ...
    private String getStatusText(TxtChapter chapter) {
        String tip = "";
        TxtChapter.Status status = chapter.getStatus();
        if (status == TxtChapter.Status.LOADING) {
            tip = mContext.getString(R.string.loading);
        } else if (status == TxtChapter.Status.ERROR) {
            tip = mContext.getString(R.string.load_error_msg, curChapter().txtChapter.getMsg());
        } else if (status == TxtChapter.Status.EMPTY) {
            tip = mContext.getString(R.string.content_empty);
        } else if (status == TxtChapter.Status.CATEGORY_EMPTY) {
            tip = mContext.getString(R.string.chapter_list_empty);
        } else if (status == TxtChapter.Status.CHANGE_SOURCE) {
